...
exports.run = async (bot, message, args) => {
    if (message.mentions.everyone) return message.channel.reply(", do not mention everyone in the message, I can't just ban everyone, you know?");
    let uId = message.mentions.members.first();
    if (!isNaN(args[1]) && args.length < 3) {
        uId.ban(args[1]);
        message.channel.send(uId + "has been banned!");
    } else if (!isNaN(args[1]) && args.length > 2) {
        uId.ban(args[1], args.slice(2).join(" "));
        message.channel.send(uId + "has been banned for \`" + args.slice(2).join(" ") + "\`!");
